...
import OptionIcon from '@material-ui/icons/SubdirectoryArrowRight';
			...
			<React.Fragment>
						...
						return view.ATTRIB_03 === 'root' ? (
							<React.Fragment>
								<Link
									...
									<ListItem button selected={this.isSelected(view.ATTRIB_01)}>
										<ListItemIcon>
											<OptionIcon />
										</ListItemIcon>
				...
				}
				{
					categories.map(category => {
						let openState = this.state[`open${capitalize(category.value)}`]
						return (
							<React.Fragment>
								<ListItem className={classes.listItem} button id={`open${capitalize(category.value)}`} onClick={this.handleClick}>
									<ListItemIcon>
										{category.icon}
									</ListItemIcon>
									<ListItemText classes={{ root: classes.listItemText }} primary={category.label} />
									{openState ? <ExpandLess /> : <ExpandMore />}
								</ListItem>
								<Collapse in={openState} timeout="auto" unmountOnExit>
									{
										this.props.views.map(view => {
											return view.ATTRIB_03 === category.value ? (
